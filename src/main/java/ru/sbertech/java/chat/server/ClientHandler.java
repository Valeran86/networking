package ru.sbertech.java.chat.server;
...
public class ClientHandler {
    ...
    private String nick;
    private long timeToCloseClient = 125_000; // Время на закрытия сокета и клиент хэндлера в миллисекундах
    private boolean is_auth =false;   // Клиент вошел в чат?
    ...
    public void settimeToCloseClient(long TimeMillisClose) {
        timeToCloseClient = TimeMillisClose;
    }
    ...
    public void closeSocket() {
        if(!socket.isClosed()) {
            ...
            System.out.println("Клиент отключен " + socket);
        }
    }
    ...
    public ClientHandler(Server server, Socket socket) {
        try {
            ...
            reader.start();
            reader.join(timeToCloseClient);   // Отключаем сокет если клиент не авторизовался за timeToCloseClient миллисекунд
            if (!is_auth && !socket.isClosed()) {
                reader.interrupt();
                closeSocket();
                in.close();
                out.close();
            ...
            reader.join(3000);
            System.out.println("Жив поток reader "+reader.isAlive()+" Авторизация "+is_auth+" Сокет закрыт "+socket.isClosed());
